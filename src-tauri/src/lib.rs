...
pub fn run() {
    tauri::Builder::default()
        ...
        .manage(TrayManagerState(std::sync::Mutex::new(tray::TrayMenuManager::new())))
        .invoke_handler(tauri::generate_handler![
            ...
            commands::runtime_installer::check_edwinpai_runtime,
            commands::runtime_installer::install_edwinpai_runtime,
            commands::runtime_installer::start_edwinpai_gateway_cli,
            ...
            tauri::async_runtime::spawn(async move {
                ...
                for url in &urls {
                    if let Ok(resp) = reqwest::get(*url).await {
                        if resp.status().is_success() || resp.status().as_u16() == 404 || resp.status().as_u16() == 426 {
