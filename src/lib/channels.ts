...
const KNOWN_CHANNELS: ChannelName[] = ['telegram', 'matrix', 'discord', 'slack', 'whatsapp', 'signal']
 ...
 */
export async function listChannelsFromGatewayConfig(): Promise<ChannelConfig[]> {
  let config: Record<string, unknown>;

  try {
    // Try to fetch from the selected gateway via WebSocket
    const { readConfig } = await import('@/lib/config');
    const { fetchGatewayConfig, inferGatewayKind } = await import('@/lib/gateway-context');
    ...
    if (desktopConfig.gatewayUrl) {
      ...
      const response = await invoke<{ config: Record<string, unknown> }>('get_edwinpai_config');
  ...
  const channelsObj = (messages.channels ?? config.channels ?? {}) as Record<string, Record<string, unknown>>
  ...
  for (const name of KNOWN_CHANNELS) {
    ...
    const metaKeys = new Set(['enabled', 'allowFrom', 'groupPolicy', 'groups', 'autoReply'])
    ...
    // Extract settings
    const groups = (channelCfg.groups ?? {}) as Record<string, Record<string, unknown>>
    ...
    const settings: ChannelSettings = {
      autoReply: channelCfg.autoReply === true ||
        Object.values(groups).some((g: Record<string, unknown>) => g.autoReply === true),
      allowedChatIds,
    }
