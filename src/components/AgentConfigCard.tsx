...
import { Bot, Brain, RefreshCw, Save, AlertCircle, CheckCircle } from 'lucide-react';
...
import { fetchGatewayConfig, listGatewayModels, patchGatewayConfig, resolveToken, type GatewayModelChoice, type GatewayTarget } from '@/lib/gateway-context';
...
export function AgentConfigCard({ gatewayUrl, gatewayToken }: AgentConfigCardProps) {
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      if (gwConfig) {
        const defaults = (gwConfig as Record<string, unknown>).agents as Record<string, unknown> | undefined;
        const agentDefaults = defaults?.defaults as Record<string, unknown> | undefined;
        const modelConfig = agentDefaults?.model as Record<string, unknown> | undefined;
        const memorySearch = agentDefaults?.memorySearch as Record<string, unknown> | undefined;

        setConfig({
          model: (modelConfig?.primary as string) ?? '',
          fallbacks: Array.isArray(modelConfig?.fallbacks) ? (modelConfig.fallbacks as string[]).join(', ') : '',
      ...
      agents: {
        defaults: {
          ...
          memorySearch: {
            ...
            ...(config.memoryProvider !== 'auto' ? { provider: config.memoryProvider } : {}),
          },
        },
      },
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={() => { loadConfig(); loadModels(); }} disabled={status === 'loading'}>
      ...
      <div className="space-y-4">
          ...
          >
            ...
            <SelectContent>
              {availableModels.map((m) => (
                <SelectItem key={m.id} value={m.id}>{m.name}{m.provider ? ` (${m.provider})` : ''}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <p className="text-xs text-muted-foreground">
            Default model for new sessions. Per-session overrides are available in Chat.
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setConfig(prev => ({ ...prev, fallbacks: e.target.value }))}
            placeholder="anthropic/claude-haiku-4-5, openai/gpt-4o-mini"
          />
          <p className="text-xs text-muted-foreground">
            Comma-separated. Used when the primary model fails or is rate-limited.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Switch
            ...
            onCheckedChange={(checked) => setConfig(prev => ({ ...prev, memoryEnabled: checked }))}
          />
        </div>
        ...
        {config.memoryEnabled && (
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="auto">Auto (use available API key)</SelectItem>
                <SelectItem value="openai">OpenAI (text-embedding-3-small)</SelectItem>
                <SelectItem value="gemini">Google Gemini</SelectItem>
                <SelectItem value="local">Local (no API key needed, ~300MB model)</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-xs text-muted-foreground">
              Embedding model for semantic memory search. Local runs on CPU with no external calls.
            </p>
          </div>
        )}
      </div>
