...
describe('IdentitySettings', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('renders identicon SVG', async () => {
      ...
      await waitFor(() => {
        const identiconContainer = screen.getByText('edw:a1b2c3d4').previousElementSibling;
        expect(identiconContainer?.innerHTML).toContain('<svg>test identicon</svg>');
      }, { timeout: 1000 });
    });
    ...
    it('shows export and import buttons', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Export Public Identity')).toBeInTheDocument();
        expect(screen.getByText('Import Identity (Coming Soon)')).toBeInTheDocument();
      }, { timeout: 1000 });
    });
    ...
    it('disables import button', async () => {
      ...
      await waitFor(() => {
        const importButton = screen.getByText('Import Identity (Coming Soon)').closest('button');
    ...
    it('displays informational card about identity', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/BRC-42 and BRC-103 standards/)).toBeInTheDocument();
      }, { timeout: 1000 });
    });
  });
  ...
  describe('Identity Display', () => {
    ...
    it('shows petname description text', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Deterministically generated from your public key')
        ).toBeInTheDocument();
      }, { timeout: 1000 });
    });
  ...
  describe('Export Functionality', () => {
    ...
    it('exports identity as JSON file', async () => {
      render(<IdentitySettings />);
      await waitFor(() => screen.getByText('Export Public Identity'), { timeout: 1000 });
    ...
    it('shows success message after export', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Identity exported successfully/i)).toBeInTheDocument();
      }, { timeout: 1000 });
    });
    ...
    it('clears success message after 3 seconds', async () => {
      ...
      await waitFor(() => screen.getByText('Export Public Identity'), { timeout: 2000 });
      ...
      await waitFor(() => {
        expect(screen.queryByText(/Identity exported successfully/i)).not.toBeInTheDocument();
      }, { timeout: 500 });
    });
    ...
    it('includes all identity fields in export', async () => {
      ...
      let blobContent = '';
      const BlobSpy = vi.spyOn(global, 'Blob').mockImplementation(function(this: Blob, content?: BlobPart[]) {
  ...
  describe('Copy Functionality', () => {
    ...
    it('handles clipboard API errors gracefully', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Failed to copy public key/)).toBeInTheDocument();
      }, { timeout: 1000 });
    });
  });
  ...
  describe('Error Handling', () => {
    it('displays error when getIdentity fails', async () => {
      vi.mocked(cryptoDomain.getIdentity).mockRejectedValue(new Error('Keypair not found'));
      render(<IdentitySettings />);
    ...
    it('displays generic error message for unknown errors', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Failed to load identity. Make sure you have a BSV keypair configured./)
        ).toBeInTheDocument();
      }, { timeout: 1000 });
    });

    it('displays error icon in error state', async () => {
      vi.mocked(cryptoDomain.getIdentity).mockRejectedValue(new Error('Test error'));
      render(<IdentitySettings />);
  ...
  describe('UI State Management', () => {
    it('does not call export when identity is null', async () => {
      ...
      vi.mocked(cryptoDomain.getIdentity).mockRejectedValue(new Error('No identity'));
      render(<IdentitySettings />);

      await waitFor(() => screen.getByText(/No identity/), { timeout: 1000 });

      expect(screen.queryByText('Export Public Identity')).not.toBeInTheDocument();
