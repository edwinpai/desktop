...
describe('GeneralSettings', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('shows gateway-specific settings in gateway mode', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByLabelText('Auto-start gateway')).toBeInTheDocument();
      }, { timeout: 1000 });
    });
  ...
  describe('IPC Integration', () => {
    ...
    it('calls save_config when gateway port changes', async () => {
      ...
      await waitFor(() => screen.getByLabelText('Gateway Port'), { timeout: 1000 });

      const portInput = screen.getByLabelText('Gateway Port') as HTMLInputElement;
      // Use fireEvent.change directly to avoid NaN from clear + type
      fireEvent.change(portInput, { target: { value: '4000' } });

      await waitFor(() => {
        const calls = mockInvoke.mock.calls.filter((call) => isSaveConfigPortCall(call, 4000));
    ...
    it('calls save_config when auto-start changes', async () => {
      ...
      await waitFor(() => screen.getByLabelText('Auto-start gateway'), { timeout: 1000 });
    ...
    it('calls set_mode when switching to client mode', async () => {
      ...
      await waitFor(() => {
        expect(mockInvoke).toHaveBeenCalledWith('set_mode', { mode: 'client' });
      }, { timeout: 1000 });
    });
    ...
    it('calls stop_gateway before switching from gateway to client mode', async () => {
      ...
      await waitFor(() => {
        expect(mockInvoke).toHaveBeenCalledWith('stop_gateway', { request: {} });
      }, { timeout: 1000 });
    });
  });
