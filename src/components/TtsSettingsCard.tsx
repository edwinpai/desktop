...
export function TtsSettingsCard({
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      const msgs = (config as Record<string, unknown>).messages as
      ...
      setStatus({
        type: "error",
        message: `Failed to load TTS config: ${err instanceof Error ? err.message : String(err)}`,
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    if (state.maxTextLength !== original.maxTextLength) {
      patch.maxTextLength =
        state.maxTextLength === "" ? undefined : Number(state.maxTextLength);
    }

    // Edge settings
    const edgePatch: Record<string, unknown> = {};
    if (state.edgeEnabled !== original.edgeEnabled)
      edgePatch.enabled = state.edgeEnabled;
    if (state.edgeVoice !== original.edgeVoice)
      edgePatch.voice = state.edgeVoice || undefined;
    if (Object.keys(edgePatch).length > 0) patch.edge = edgePatch;

    // OpenAI settings
    const openaiPatch: Record<string, unknown> = {};
    if (state.openaiVoice !== original.openaiVoice)
      openaiPatch.voice = state.openaiVoice;
    if (state.openaiModel !== original.openaiModel)
      openaiPatch.model = state.openaiModel;
    if (Object.keys(openaiPatch).length > 0) patch.openai = openaiPatch;

    // ElevenLabs settings
    const elPatch: Record<string, unknown> = {};
    if (state.elevenlabsVoiceId !== original.elevenlabsVoiceId)
      elPatch.voiceId = state.elevenlabsVoiceId || undefined;
    if (state.elevenlabsModelId !== original.elevenlabsModelId)
      elPatch.modelId = state.elevenlabsModelId || undefined;
    ...
    try {
      ...
      setStatus({
        type: "success",
        message: "TTS settings saved. Gateway reloading.",
      });
      setOriginal({ ...state });
    } catch (err) {
      setStatus({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Configure voice responses. EdwinPAI can read replies aloud using
          various TTS engines.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={state.auto}
            onValueChange={(v) => setState((prev) => ({ ...prev, auto: v }))}
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="inbound">
                Inbound — Only when user sends voice
              </SelectItem>
              <SelectItem value="tagged">
                Tagged — Only [[tts:...]] tagged replies
              </SelectItem>
            </SelectContent>
          </Select>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  setState((prev) => ({ ...prev, edgeEnabled: checked }))
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setState((prev) => ({ ...prev, edgeVoice: v }))
                }
              >
          ...
          <div className="space-y-4 pl-2 border-l-2 border-muted">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setState((prev) => ({ ...prev, openaiVoice: v }))
                }
              >
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setState((prev) => ({ ...prev, openaiModel: v }))
                }
              >
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">
            Longer text is auto-summarized before TTS. Leave empty for default
            (1500).
          </p>
        </div>
