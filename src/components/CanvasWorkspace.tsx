...
function supportsCanvas(node: NodeListNode): boolean {
  ...
  return (
    (node.caps ?? []).includes("canvas") ||
    commands.some((command) => command.startsWith("canvas."))
  );
}

function supportsCanvasCommand(
...
function toUserFacingError(err: unknown): string {
  ...
  if (
    msg.includes("requires a signed request") ||
    msg.includes("Missing authentication headers")
  ) {
...
export function CanvasWorkspace({
  ...
  const sessionCanvasDir = useMemo(
    () => getSessionCanvasDir(sessionKey),
    [sessionKey],
  );
  ...
  const loadNodes = useCallback(async () => {
    ...
    try {
      ...
      if (nextCanvasNodes.length === 0) {
        ...
        const stillValid = nextCanvasNodes.some(
          (node) => node.nodeId === selectedNodeId,
        );
        if (!stillValid) {
          const preferred =
            nextCanvasNodes.find((node) => node.connected) ??
            nextCanvasNodes[0];
  ...
  const runCanvasAction = useCallback(
    async (
      ...
      setError(null);
      try {
        const res = await invokeCanvasTool(
          target,
          action,
          { node: selectedNodeId, ...params },
          60_000,
        );
        setResult(res);
  ...
  const canControlSelectedNode = Boolean(
    selectedNodeId && selectedNode?.connected,
  );
  ...
  const canPushA2UI = supportsCanvasCommand(
    selectedNode,
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground max-w-3xl">
            Render and inspect visual surfaces on paired nodes. Use Canvas to
            preview A2UI, open a node surface, navigate it, run small checks,
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <Card>
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            <p>
              • Preview A2UI interfaces locally before pushing them to a device.
            </p>
            <p>• Open or navigate a live surface on a paired node.</p>
            <p>
              • Capture snapshots to verify what the node is actually showing.
            </p>
            <p>
              • Run lightweight JavaScript checks against the current surface
              when debugging.
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3">
            {canvasNodes.length > 0 ? (
                  ...
                  <div className="rounded-md border bg-muted/20 p-3 text-sm space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-medium">
                        {selectedNode.displayName ?? selectedNode.nodeId}
                      </span>
                    ...
                    <div className="text-muted-foreground">
                      ...
                      {selectedNode.remoteIp
                        ? ` · ${selectedNode.remoteIp}`
                        : ""}
                    </div>
                    {!selectedNode.connected && (
                      <p className="text-xs text-amber-600 dark:text-amber-400">
                        This node is known but not currently connected, so live
                        Canvas actions will fail until it reconnects.
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
                No canvas-capable node is connected right now. Pair a device
                with canvas support first, then come back here.
              </div>
            )}

            <div className="rounded-md border bg-muted/20 p-3 text-xs text-muted-foreground">
              <div>
                <strong>Session canvas directory:</strong> {sessionCanvasDir}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <TabsContent value="control" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                These actions talk to the selected node over the signed gateway
                control path.
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      onClick={() =>
                        void runCanvasAction(
                          "present",
                          surfaceUrl.trim()
                            ? { target: surfaceUrl.trim() }
                            : {},
                        )
                      }
                      disabled={
                        !canControlSelectedNode ||
                        !canPresent ||
                        busyAction !== null
                      }
                    >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      onClick={() =>
                        void runCanvasAction("navigate", { url: navigateUrl })
                      }
                      disabled={
                        !canControlSelectedNode ||
                        !canNavigate ||
        ...
        <TabsContent value="a2ui" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Preview JSONL locally, then push it to the selected node when
          ...
          {showA2UIPreview && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  This preview is local to the desktop tab. Use “Send to
                  selected node” to mirror it onto a device surface.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div
        ...
        <TabsContent value="advanced" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Useful when you need to inspect the current node surface or
                quickly debug what is loaded.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  disabled={
                    !canControlSelectedNode || !canEval || busyAction !== null
                  }
                >
                ...
                <Button
                  variant="outline"
                  onClick={() =>
                    void runCanvasAction("snapshot", { outputFormat: "png" })
                  }
                  disabled={
                    !canControlSelectedNode ||
                    !canSnapshot ||
      ...
      {lastSnapshot && (
        <Card>
          ...
          <CardContent>
            <img
              src={lastSnapshot}
              alt="Latest canvas snapshot"
              className="max-h-[32rem] rounded border"
            />
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Raw result from the most recent action. Helpful when you are
