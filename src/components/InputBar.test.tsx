...
describe('InputBar', () => {
  ...
  it('renders with default placeholder', () => {
    ...
    expect(screen.getByPlaceholderText('Type a message... (! for shell commands, / for slash commands)')).toBeInTheDocument();
  });

  it('renders with custom placeholder', () => {
    render(<InputBar onSendMessage={mockOnSendMessage} placeholder="Custom placeholder" />);
  ...
  it('sends message on Enter key press', async () => {
    ...
    expect(mockOnSendMessage).toHaveBeenCalledWith('Hello world', { attachments: [] });
  });
  ...
  it('trims whitespace before sending', async () => {
    ...
    expect(mockOnSendMessage).toHaveBeenCalledWith('Message with spaces', { attachments: [] });
  });
  ...
  it('sends message on send button click', async () => {
    ...
    expect(mockOnSendMessage).toHaveBeenCalledWith('Click to send', { attachments: [] });
  });
  ...
  it('still sends messages when disabled prop is true', async () => {
    ...
    expect(mockOnSendMessage).toHaveBeenCalledWith('Should still send', { attachments: [] });
  });
  ...
  it('restores unsent draft text for the same session key after remount', async () => {
    const user = userEvent.setup();
    const { unmount } = render(<InputBar onSendMessage={mockOnSendMessage} sessionKey="agent:main:main" />);

    const textarea = screen.getByRole('textbox');
    await user.type(textarea, 'Draft that should survive tab switching');

    expect(window.localStorage.getItem('edwinpai:chat:draft:agent:main:main')).toBeNull();

    unmount();

    expect(window.localStorage.getItem('edwinpai:chat:draft:agent:main:main')).toBe('Draft that should survive tab switching');

    render(<InputBar onSendMessage={mockOnSendMessage} sessionKey="agent:main:main" />);

    expect(screen.getByRole('textbox')).toHaveValue('Draft that should survive tab switching');
  });
  ...
  it('debounces local text persistence while typing without syncing draft upstream', () => {
    ...
    expect(handleDraftChange).not.toHaveBeenCalled();
    expect(window.localStorage.getItem('edwinpai:chat:draft:agent:main:main')).toBe('Draft');

    unmount();
    expect(handleDraftChange).toHaveBeenCalledWith({ value: 'Draft', attachments: [] });
  ...
  it('preserves attachments and text across remount when draft state is hoisted', async () => {
    ...
    function Harness() {
      const [draft, setDraft] = useState({ value: '', attachments: [] as Array<{ type?: string; mimeType: string; fileName: string; content: string }> });
      const [mounted, setMounted] = useState(true);
      const handleDraftChange = useCallback((next: { value: string; attachments: Array<{ type?: string; mimeType: string; fileName: string; content: string }> }) => {
        setDraft(next);
      }, []);

      return (
        <div>
          <button type="button" onClick={() => setMounted((prev) => !prev)}>toggle</button>
          {mounted ? (
            <InputBar
              onSendMessage={mockOnSendMessage}
              sessionKey="agent:main:main"
