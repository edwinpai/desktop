...
export function AccessControl({
  ...
  const loadIdentity = useCallback(async () => {
    try {
      ...
      setLocalIdentity({
        publicKey: identity.public_key || undefined,
  ...
  const handleSelfCheck = async () => {
    ...
    try {
      ...
      if (
        verifyResult.valid &&
        signResult.public_key === localIdentity?.publicKey
      ) {
        setSelfCheckResult({
          status: "pass",
  ...
  const refreshInvitations = async () => {
    try {
      const response = await invoke<{ invitations: Invitation[] }>(
        "list_invitations",
        {
          request: {},
        },
      ...
      setActiveInvitations(
        response.invitations.filter((inv) => inv.status === "pending"),
      );
    } catch (err) {
    ...
    try {
      ...
      if (!localIdentity?.publicKey) {
        setRedeemResult({
          success: false,
          message:
            "No identity key found. Generate an identity first in Settings.",
        });
      ...
      setRedeemResult({
        ...
        message:
          response.message +
          (response.accessLevel ? ` (Level: ${response.accessLevel})` : ""),
      });
      ...
      <Card className="p-8">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Only gateway owners can manage user access.
          </p>
        </div>
      </Card>
    );
        ...
        {localIdentity?.publicKey ? (
          <div className="space-y-4">
            <div className="grid gap-3">
                ...
                <div className="flex items-center gap-2">
                  <span
                    ...
                    dangerouslySetInnerHTML={{
                      __html: localIdentity.avatarSvg,
                    }}
                  />
              ...
              <div className="flex items-center justify-between p-3 rounded-md bg-muted">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs">
                    {formatPublicKey(localIdentity.publicKey)}
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
              ...
              <div className="flex items-center justify-between p-3 rounded-md bg-muted">
                ...
                <span className="font-mono text-xs">
                  OS Keychain (com.edwinpai.desktop)
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t">
              <div className="flex items-center gap-2">
                ...
                {selfCheckResult.status === "idle" && (
                  <span className="text-sm text-muted-foreground">
                    Run self-check to verify key integrity
                  </span>
                )}
              </div>
              <Button
          ...
          <div className="p-4 rounded-md bg-destructive/10 text-center">
            <p className="text-sm text-destructive mb-2">
              Identity key not found in keychain.
            </p>
            <p className="text-xs text-muted-foreground">
              Generate an identity during onboarding or in Settings.
            </p>
          </div>
        ) : localIdentity ? (
          <div className="p-4 rounded-md bg-yellow-500/10 text-center">
            <p className="text-sm text-yellow-700 dark:text-yellow-300 mb-2">
              Local identity loaded, but no public key is currently available.
            </p>
            <p className="text-xs text-muted-foreground">
              Check keychain access or regenerate the local identity.
            </p>
          </div>
        ) : (
        ...
        <Card className="p-6">
          ...
          {localIdentity?.publicKey ? (
            <div className="mb-4 p-3 rounded-md bg-muted text-sm space-y-1">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-xs">
                  BSV/secp256k1 (Type-42)
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Public Key</span>
                <span className="font-mono text-xs">
                  {formatPublicKey(localIdentity.publicKey, 8, 8)}
                </span>
              </div>
              <div className="flex items-center justify-between">
            ...
            <div className="mb-4 p-3 rounded-md bg-destructive/10 text-sm">
              <p className="text-destructive">
                No identity key found. Generate an identity in Settings first.
              </p>
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            <Button
              ...
              disabled={
                isRedeeming || !redeemInput.trim() || !localIdentity?.publicKey
              }
              className="w-full"
              variant="secondary"
            >
                ...
                <Card key={invitation.token} className="p-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {revokingInvite === invitation.token
                          ? "Revoking..."
                          : "Revoke"}
                      </Button>
                    </div>
          ...
          {authorizedUsers.length === 0 ? (
            <Card className="p-8">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  No authorized users yet. Create an invitation and redeem it to
                  add users.
                </p>
              </div>
            </Card>
