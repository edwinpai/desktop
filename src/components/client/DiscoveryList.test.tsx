...
describe('DiscoveryList', () => {
  ...
  it('shows scanning indicator when isScanning=true and no peers', () => {
    const { container } = render(<DiscoveryList peers={[]} onSelect={vi.fn()} isScanning={true} />);
  ...
  it('renders single peer with all details', () => {
    render(<DiscoveryList peers={[mockPeer]} onSelect={vi.fn()} isScanning={false} />);
  ...
  it('renders multiple peers', () => {
    ...
    render(<DiscoveryList peers={peers} onSelect={vi.fn()} isScanning={false} />);
  ...
  it('handles peer with empty petname', () => {
    ...
    const { container } = render(<DiscoveryList peers={[peerWithoutName]} onSelect={vi.fn()} />);
  ...
  it('displays peers while still scanning', () => {
    render(<DiscoveryList peers={[mockPeer]} onSelect={vi.fn()} isScanning={true} />);

    // Should show peer, not scanning indicator
    expect(screen.getByText('alice-gateway')).toBeInTheDocument();
    expect(screen.queryByText('Scanning for gateways...')).not.toBeInTheDocument();
  });
