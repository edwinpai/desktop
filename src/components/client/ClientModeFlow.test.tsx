...
type DiscoveryListMockProps = {
  peers: DiscoveredPeer[];
  onSelect: (peer: DiscoveredPeer) => void;
  isScanning: boolean;
};
type ConnectionStatusMockProps = {
  status: string;
...
describe("ClientModeFlow", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(useDiscovery).mockReturnValue(
      mockUseDiscovery as unknown as ReturnType<typeof useDiscovery>,
    );
    vi.mocked(useClientConnection).mockReturnValue(
      mockUseClientConnection as unknown as ReturnType<
        typeof useClientConnection
      >,
    );
  });
  ...
  it("handles multiple peers", () => {
    ...
    expect(
      screen.getByRole("button", { name: /alice-gateway/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /bob-gateway/i }),
    ).toBeInTheDocument();
  });
});
