...
describe("ConnectionStatus", () => {
  ...
  it("renders connected state with default message", () => {
    ...
    expect(
      screen.getByText("Successfully connected to gateway"),
    ).toBeInTheDocument();
  });
  ...
  it("renders connected state with gateway name", () => {
    ...
    expect(screen.getByText("Connected")).toBeInTheDocument();
    expect(
      screen.getByText("Successfully connected to alice-gateway"),
    ).toBeInTheDocument();
  });
  ...
  it("shows encryption badge when connected", () => {
    ...
    expect(
      screen.getByText("Encrypted connection established"),
    ).toBeInTheDocument();
  ...
  it("renders connecting state with default message", () => {
    ...
    expect(
      screen.getByText("Authenticating with gateway..."),
    ).toBeInTheDocument();
  });
  ...
  it("renders connecting state with gateway name", () => {
    ...
    expect(screen.getByText("Connecting")).toBeInTheDocument();
    expect(
      screen.getByText("Authenticating with bob-gateway..."),
    ).toBeInTheDocument();
  });
  ...
  it("renders failed state with default error message", () => {
    ...
    expect(
      screen.getByText("Failed to connect to gateway"),
    ).toBeInTheDocument();
  });
  ...
  it("renders disconnected state", () => {
    ...
    expect(
      screen.getByText("Not connected to any gateway"),
    ).toBeInTheDocument();
  });
  ...
  it("shows error message separately when not in failed state", () => {
    render(
      <ConnectionStatus status="connecting" error="Temporary network issue" />,
    );
  ...
  it("handles error prop when no error is provided", () => {
    ...
    expect(
      screen.queryByText("Temporary network issue"),
    ).not.toBeInTheDocument();
  });
  ...
  it("handles undefined status as disconnected", () => {
    render(
      <ConnectionStatus
        status={undefined as unknown as ClientConnectionStatus}
      />,
    );
  ...
  it("handles invalid status as disconnected", () => {
    render(
      <ConnectionStatus status={"invalid-status" as ClientConnectionStatus} />,
    );
  ...
  it("truncates very long gateway names in message", () => {
    const longName =
      "this-is-an-extremely-long-gateway-name-that-might-break-layout";
    render(<ConnectionStatus status="connected" gatewayName={longName} />);

    expect(
      screen.getByText(`Successfully connected to ${longName}`),
    ).toBeInTheDocument();
  });
