...
describe("TelegramWizard", () => {
  ...
  describe("Intro Step", () => {
    it("should render intro step", () => {
      render(
        <TelegramWizard onComplete={mockOnComplete} onCancel={mockOnCancel} />,
      );

      expect(screen.getByText("Connect Telegram Bot")).toBeInTheDocument();
      expect(
        screen.getByText(/This wizard will help you connect a Telegram bot/i),
      ).toBeInTheDocument();
    });
    ...
    it("should display instructions for creating bot", () => {
      ...
      expect(
        screen.getByText(/Open Telegram and search for @BotFather/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Send \/newbot and follow the prompts/i),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Credentials Step Validation", () => {
    it("should validate bot token format (BOT_ID:AUTH_TOKEN)", async () => {
      ...
      await user.type(
        input,
        "123456789:ABCdefGHIjklMNOpqrsTUVwxyz012345678w11",
      );
