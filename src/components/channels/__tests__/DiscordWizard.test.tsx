...
describe('DiscordWizard', () => {
  ...
  describe('Intro Step', () => {
    it('should render intro step', () => {
      render(<DiscordWizard onComplete={mockOnComplete} onCancel={mockOnCancel} />);

      expect(screen.getByText('Connect Discord Bot')).toBeInTheDocument();
      expect(screen.getByText(/This wizard will help you connect a Discord bot/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Credentials Step', () => {
    it('should render bot token tab', async () => {
      ...
      expect(screen.getByRole('tab', { name: 'Bot Token' })).toBeInTheDocument();
    ...
    it('should render OAuth tab', async () => {
      ...
      const oauthTab = screen.getByRole('tab', { name: /OAuth \(Coming Soon\)/i });
    ...
    it('should validate bot token length (≥50 chars)', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Bot token appears too short. Please check and try again.')).toBeInTheDocument();
      });
    });
  ...
  describe('Validation Step', () => {
    it('should call validateCredentials with bot token', async () => {
      ...
      const input = screen.getByPlaceholderText(/MTk4NjIyNDgz/i);
      await user.type(input, 'MTIzNDU2Nzg5MDEyMzQ1Njc4OQ.GhIjKlMnOpQrStUvWxYz123456');
      ...
      await waitFor(() => {
        expect(screen.getByText(/Validating your bot token/i)).toBeInTheDocument();
      });
    ...
    it('should display validation metadata (authMethod)', async () => {
      ...
      mockValidateCredentials.mockResolvedValue({
        valid: true,
        metadata: { botId: '123456789', username: 'TestBot', discriminator: '1234' },
      });
