...
describe("SlackWizard", () => {
  ...
  describe("Intro Step", () => {
    it("should render intro step", () => {
      render(
        <SlackWizard onComplete={mockOnComplete} onCancel={mockOnCancel} />,
      );

      expect(screen.getByText("Connect Slack Workspace")).toBeInTheDocument();
      expect(
        screen.getByText(
          /This wizard will help you connect to a Slack workspace/i,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Credentials Step Validation", () => {
    ...
    it("should validate xoxb- prefix", async () => {
      ...
      await user.type(
        input,
        "invalid-prefix-1234567890-1234567890-abcdefghijklmnopqrstuvwxyz",
      );
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Invalid token format.*xoxb-.*xoxp-/i),
        ).toBeInTheDocument();
      });
    });
    ...
    it("should accept xoxb- prefix", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText(/Invalid token format/i),
    ...
    it("should accept xoxp- prefix", async () => {
      ...
      await user.type(
        input,
        "xoxp-1234567890-1234567890-abcdefghijklmnopqrstuvwxyz",
      );
    ...
    it("should validate token length (≥40 chars)", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Testing your Slack access token/i),
        ).toBeInTheDocument();
      });
    });
  });
