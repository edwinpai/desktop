...
describe('SignalWizard', () => {
  ...
  describe('Intro Step', () => {
    it('should render intro step', () => {
      render(<SignalWizard onComplete={mockOnComplete} onCancel={mockOnCancel} />);

      expect(screen.getByText('Connect Signal Account')).toBeInTheDocument();
      expect(screen.getByText(/This wizard will help you connect your Signal account/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Credentials Step Validation', () => {
    ...
    it('should validate JSON structure', async () => {
      ...
      fireEvent.change(textarea, { target: { value: '{"deviceId": 1, "registrationId": 12345}' } });
    ...
    it('should show error for non-object JSON', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Invalid device data structure/i)).toBeInTheDocument();
      });
    });
  ...
  describe('Validation Step', () => {
    it('should call validateCredentials on validation step', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Testing your Signal device data/i)).toBeInTheDocument();
      });
  ...
  describe('Confirmation Step', () => {
    it('should create channel on completion', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Signal channel configured successfully/i)).toBeInTheDocument();
      });
