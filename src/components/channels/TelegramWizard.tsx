...
export function TelegramWizard({ channel = 'telegram', onComplete, onCancel, existingConfig }: ChannelWizardProps) {
  const [currentStepIndex, setCurrentStepIndex] = useState(0)
  const [botToken, setBotToken] = useState(existingConfig?.credentials?.botToken || '')
  const [error, setError] = useState<string>()
  const [loading, setLoading] = useState(false)
  const [validationMetadata, setValidationMetadata] = useState<{ botId?: string; username?: string }>()
    ...
    content: (
      <div className="space-y-4">
        ...
        <ul className="list-disc list-inside space-y-2 text-sm text-muted-foreground">
          ...
          <li>Token format: BOT_ID:AUTH_TOKEN (e.g., 123456789:ABCdefGHIjklMNOpqrsTUVwxyz)</li>
        </ul>
        <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg space-y-2">
          <p className="text-sm font-medium text-blue-900 dark:text-blue-100">How to create a bot:</p>
  ...
  const credentialsStep: WizardStepConfig = {
      ...
      <div className="space-y-4">
        ...
        {botToken && !validateTokenFormat(botToken) && (
          <div className="text-sm text-orange-600 dark:text-orange-400">
            Token format appears invalid. Expected format: 123456789:ABCdefGHIjklMNOpqrsTUVwxyz
          </div>
        )}
      </div>
    ),
    onValidate: async () => {
      ...
      if (!validateTokenFormat(botToken)) {
        setError('Invalid token format. Expected: BOT_ID:AUTH_TOKEN (e.g., 123456789:ABCdefGHI...)')
        return false
      }
        ...
        {validationMetadata ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
              ...
              <span className="text-sm font-medium">Bot validated successfully</span>
            </div>
            {validationMetadata.botId && (
              <div className="text-sm text-muted-foreground">
                Bot ID: <span className="font-mono">{validationMetadata.botId}</span>
              </div>
            )}
            {validationMetadata.username && (
              <div className="text-sm text-muted-foreground">
                Username: <span className="font-mono">@{validationMetadata.username}</span>
              </div>
            )}
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">Validating your bot token...</p>
        )}
      </div>
    ),
    onValidate: async () => {
  ...
  const confirmationStep: WizardStepConfig = {
    ...
    content: (
      <div className="space-y-4">
        <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
          ...
          <span className="font-medium">Telegram bot configured successfully</span>
        </div>
    ...
    onValidate: async () => {
      ...
      try {
        ...
        onComplete?.({ enabled: true, botToken } as unknown as import("@/types/channels").ChannelConfig)
        return true
      } catch (err) {
