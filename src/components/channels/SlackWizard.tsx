...
export function SlackWizard({ channel = 'slack', onComplete, onCancel, existingConfig }: ChannelWizardProps) {
  const [currentStepIndex, setCurrentStepIndex] = useState(0)
  const [accessToken, setAccessToken] = useState(existingConfig?.credentials?.accessToken || '')
    ...
    content: (
      <div className="space-y-4">
        ...
        <ul className="list-disc list-inside space-y-2 text-sm text-muted-foreground">
          ...
          <li>Token must have appropriate scopes (chat:write, channels:read, etc.)</li>
          <li>App must be installed to your workspace</li>
        </ul>
        <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg space-y-2">
          <p className="text-sm font-medium text-blue-900 dark:text-blue-100">How to get a token:</p>
          <ol className="list-decimal list-inside space-y-1 text-sm text-blue-800 dark:text-blue-200">
            <li>Go to https://api.slack.com/apps</li>
            <li>Create a new app or select an existing one</li>
            <li>Go to "OAuth & Permissions"</li>
            <li>Add required scopes (chat:write, channels:read, users:read, etc.)</li>
        ...
        <div className="bg-orange-50 dark:bg-orange-950 p-3 rounded-lg">
          <p className="text-xs text-orange-900 dark:text-orange-100">
            <Lightbulb className="h-4 w-4 inline mr-1" /> Tip: Bot tokens (xoxb-) are recommended for automated messaging
          </p>
        </div>
      </div>
    ),
  ...
  const credentialsStep: WizardStepConfig = {
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Bot tokens start with <code className="bg-muted px-1 py-0.5 rounded">xoxb-</code>, user tokens with{' '}
        ...
        {accessToken && !validateTokenPrefix(accessToken) && (
          <div className="text-sm text-orange-600 dark:text-orange-400">
            <AlertTriangle className="h-4 w-4 inline text-amber-500 mr-1" /> Token should start with xoxb- (bot) or xoxp- (user)
          </div>
        )}

        {accessToken && validateTokenPrefix(accessToken) && (
          <div className="text-sm text-green-600 dark:text-green-400">
            <CheckCircle2 className="h-4 w-4 inline text-green-500 mr-1" /> Token type: {getTokenType(accessToken)}
          </div>
        )}
      </div>
    ),
    onValidate: async () => {
      ...
      if (!validateTokenPrefix(accessToken)) {
        setError('Invalid token format. Token must start with xoxb- (bot) or xoxp- (user)')
        return false
      }
        ...
        {validationMetadata ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
              ...
              <span className="text-sm font-medium">Token validated successfully</span>
            </div>
            {validationMetadata.tokenType && (
              <div className="text-sm text-muted-foreground">
                Token type: <span className="font-medium">{validationMetadata.tokenType}</span>
              </div>
            )}
            {validationMetadata.teamName && (
              <div className="text-sm text-muted-foreground">
                Workspace: <span className="font-medium">{validationMetadata.teamName}</span>
              </div>
            )}
            {validationMetadata.botUserId && (
              <div className="text-sm text-muted-foreground">
                Bot User ID: <span className="font-mono">{validationMetadata.botUserId}</span>
              </div>
            )}
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">Validating your token...</p>
        )}
      </div>
    ),
    onValidate: async () => {
  ...
  const confirmationStep: WizardStepConfig = {
    ...
    content: (
      <div className="space-y-4">
        <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
          ...
          <span className="font-medium">Slack workspace configured successfully</span>
        </div>
        <div className="bg-muted p-4 rounded-lg space-y-2">
          ...
          {validationMetadata?.tokenType && (
            <div className="text-sm">
              ...
              <span className="font-medium">{validationMetadata.tokenType}</span>
            </div>
          )}
          {validationMetadata?.teamName && (
            <div className="text-sm">
        ...
        <div className="bg-blue-50 dark:bg-blue-950 p-3 rounded-lg">
          <p className="text-xs text-blue-900 dark:text-blue-100">
            <Lightbulb className="h-4 w-4 inline mr-1" /> Channel picker will be available in Phase 6 for selecting specific Slack channels
          </p>
        </div>
      </div>
    ),
    onValidate: async () => {
      ...
      try {
        ...
        onComplete?.({ enabled, botToken: accessToken } as unknown as import("@/types/channels").ChannelConfig)
        return true
      } catch (err) {
