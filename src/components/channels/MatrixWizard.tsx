...
export function MatrixWizard({ channel = 'matrix', onComplete, onCancel, existingConfig }: ChannelWizardProps) {
  const [currentStepIndex, setCurrentStepIndex] = useState(0)
  const [authMethod, setAuthMethod] = useState<AuthMethod>(
    existingConfig?.credentials?.accessToken ? 'token' : 'password'
  )
  const [homeserver, setHomeserver] = useState(existingConfig?.credentials?.homeserver || 'https://matrix.org')
  const [accessToken, setAccessToken] = useState(existingConfig?.credentials?.accessToken || '')
  const [username, setUsername] = useState(existingConfig?.credentials?.username || '')
  const [password, setPassword] = useState(existingConfig?.credentials?.password || '')
    ...
    content: (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">
          This wizard will help you connect to a Matrix homeserver. You can authenticate using:
        </p>
        <ul className="list-disc list-inside space-y-2 text-sm text-muted-foreground">
          <li>
            <strong>Access Token:</strong> Pre-generated token from your Matrix account settings
          </li>
          <li>
            <strong>Username/Password:</strong> Your Matrix credentials (token will be generated)
          </li>
        </ul>
        <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg space-y-2">
          <p className="text-sm font-medium text-blue-900 dark:text-blue-100">Supported homeservers:</p>
          ...
          <div className="space-y-3">
            ...
            {validationMetadata.homeserver && (
              <div className="text-sm text-muted-foreground">
                Homeserver: <span className="font-medium">{validationMetadata.homeserver}</span>
              </div>
            )}
            {validationMetadata.userId && (
              <div className="text-sm text-muted-foreground">
                User ID: <span className="font-mono">{validationMetadata.userId}</span>
              </div>
            )}
            <div className="text-sm text-muted-foreground">
              Auth method: <span className="font-medium">{validationMetadata.authMethod}</span>
            </div>
          </div>
        ) : (
    ...
    onValidate: async () => {
      ...
      try {
        ...
        setValidationMetadata({
          ...
          authMethod: authMethod === 'token' ? 'Access Token' : 'Username/Password',
        })
  ...
  const confirmationStep: WizardStepConfig = {
    ...
    content: (
      <div className="space-y-4">
        <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
          ...
          <span className="font-medium">Matrix channel configured successfully</span>
        </div>
        <div className="bg-muted p-4 rounded-lg space-y-2">
          ...
          {validationMetadata?.homeserver && (
            <div className="text-sm">
              ...
              <span className="font-medium">{validationMetadata.homeserver}</span>
            </div>
          )}
          {validationMetadata?.userId && (
            <div className="text-sm">
          ...
          <div className="text-sm">
            ...
            <span className="font-medium">{validationMetadata?.authMethod}</span>
          </div>
          <div className="text-sm">
    ...
    onValidate: async () => {
      ...
      try {
        ...
        onComplete?.({ enabled: true, ...channelConfig } as unknown as import("@/types/channels").ChannelConfig)
        return true
      } catch (err) {
